...
namespace TenmoServer.DAO
{
    public class UserSqlDao : IUserDao
    {
        ...
        public List<UserTEBucks> GetSendTeBucksList(int userId) {
            List<UserTEBucks> returnUsers = new List<UserTEBucks>();

            try {
                using (SqlConnection conn = new SqlConnection(connectionString)) {
                    conn.Open();
                    var sql = 
                        "SELECT u.user_id, u.username, a.account_id " +
                        "FROM users u " +
                    ...
                    while (reader.Read()) {
                        UserTEBucks u = GetUserTEBucksFromReader(reader);
                        returnUsers.Add(u);
                    }
                }
            } catch (SqlException) {
        ...
        private UserTEBucks GetUserTEBucksFromReader(SqlDataReader reader)
        {
            UserTEBucks u = new UserTEBucks()
            {
                UserId = Convert.ToInt32(reader["user_id"]),
                Username = Convert.ToString(reader["username"]),
